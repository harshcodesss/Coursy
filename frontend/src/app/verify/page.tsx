...
import React, { useState, useRef, ChangeEvent, KeyboardEvent, ClipboardEvent, FormEvent, useEffect, Suspense } from "react";
...
function VerifyContent() {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/users/resendOTP`, { 
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      >
        ...
        <form onSubmit={handleVerify}>
          <div className="flex justify-center gap-3 my-8" onPaste={handlePaste}>
            {otp.map((value, index) => (
              <input
                ...
                className="w-14 h-14 bg-black/30 border border-white/20 rounded-lg text-center text-2xl font-semibold text-white transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 placeholder:text-slate-600"
              />
            ))}
          </div>

          {error && <p className="text-center text-sm text-red-400 mb-4">{error}</p>}
          {info && <p className="text-center text-sm text-green-400 mb-4">{info}</p>}
          ...
          <div className="flex flex-col gap-4">
            <motion.button
              ...
              className="w-full rounded-lg py-3 font-bold text-black transition-all duration-200 bg-gradient-to-r from-cyan-400 to-teal-400 hover:shadow-lg hover:shadow-cyan-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.98 }}
              className="w-full rounded-lg py-3 font-medium text-slate-300 transition-all duration-200 border border-white/20 bg-black/20 hover:bg-white/10 hover:text-white"
            >
              Verify Later
            </motion.button>
          </div>
        ...
        <p className="text-center text-sm text-slate-400 mt-6">
          ...
          <button
            ...
            className="font-medium text-cyan-400 hover:text-cyan-300 transition-colors duration-150 disabled:text-slate-500 disabled:cursor-not-allowed"
          >
