...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ...
      ref={ref}
      className={cn(
        // FIXED: Swapped light bg-muted for dark bg-zinc-800 and hover:bg-zinc-600
        "bg-zinc-800 absolute left-1.5 h-full w-px rounded-md py-3 duration-300 ease-in-out hover:bg-zinc-600 rtl:right-1.5",
        className
      )}
      {...props}
    />
    ...
    return (
      ...
      >
        <AccordionPrimitive.Trigger
          className={cn(
            // FIXED: Added padding, transitions, and proper dark mode hover/selected states
            `flex items-center gap-2 rounded-md px-2 py-1.5 text-sm transition-all duration-200 w-full text-left`,
            className,
            {
              "bg-zinc-800 text-white font-medium": isSelected && isSelectable,
              "text-zinc-400 hover:bg-zinc-800/50 hover:text-zinc-200": !isSelected && isSelectable,
        ...
        >
          {expandedItems?.includes(value)
            ? (openIcon ?? <FolderOpenIcon className={cn("size-4", isSelected ? "text-teal-400" : "text-zinc-500")} />)
            : (closeIcon ?? <FolderIcon className={cn("size-4", isSelected ? "text-teal-400" : "text-zinc-500")} />)}
          <span>{element}</span>
        </AccordionPrimitive.Trigger>
  ...
  (
    ...
    return (
      <button
        ...
        className={cn(
          // FIXED: Added padding, transitions, and proper dark mode hover/selected states
          "flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-sm transition-all duration-200 ease-in-out text-left rtl:pr-0 rtl:pl-1",
          {
            "bg-zinc-800 text-white font-medium": isSelected && isSelectable,
            "text-zinc-400 hover:bg-zinc-800/50 hover:text-zinc-200": !isSelected && isSelectable,
          },
        ...
        {...props}
      >
        {fileIcon ?? <FileIcon className={cn("size-4", isSelected ? "text-teal-400" : "text-zinc-500")} />}
        {children}
      </button>
    )
  }
...
>(({ className, elements, expandAll = false, children, ...props }, ref) => {
  ...
  return (
    <Button
      variant={"ghost"}
      className={cn("absolute right-2 bottom-1 h-8 w-fit p-1 text-zinc-400 hover:text-white hover:bg-zinc-800", className)}
      onClick={
